...
app.patch("/api/admin/users/:id/role", adminOnly, async (req, res) => {
  try {
    ...
    if (r.rowCount === 0) return res.status(404).json({ error: "Користувача не знайдено" });
    await logAction("З��іна ролі користувача", req.user.id, `user #${userId} → ${role}`);
...
app.patch("/api/zavuch/teachers/:id", zavuchOnly, async (req, res) => {
  try {
    ...
    await logAction(
      confirmed ? "Завуч підтвердив вчителя" : "Завуч зня�� підтвердження вчителя",
      req.user.id,
      `teacher #${id}`
    );
...
//  коментарі, статистика, профіль. Усі дані читаються/пишуться у PostgreSQL.
// ============================================================================
const juryOnly = [authRequired, roleRequired("jury", "admin", "system")];

// Чи призначено поточного суддю до конкурсу.
// Для admin/system доступ дозволено завжди (привілейовані ролі).
async function juryAssigned(req, competitionId) {
...
// --- Dashboard: зведена статистика судді --------------------------------------
app.get("/api/jury/stats", juryOnly, async (req, res) => {
  try {
    ...
    const competitionsQ = privileged
      ...
      : pool.query(
          "SELECT COUNT(*)::int AS c FROM competition_judges WHERE user_id = $1",
          [uid]
        );

    // Усього заявок у доступних конкурсах
    const applicationsQ = privileged
    ...
    // Скільки заявок оцінив саме цей суддя
    const scoredQ = pool.query(
      "SELECT COUNT(DISTINCT application_id)::int AS c FROM results WHERE judge_id = $1",
      [uid]
    );

    // Середній бал, який виставив суддя
    const avgQ = pool.query(
      "SELECT COALESCE(ROUND(AVG(score)::numeric, 1), 0) AS a FROM results WHERE judge_id = $1",
      [uid]
    );

    const [competitions, applications, scored, avg] = await Promise.all([
      competitionsQ,
      applicationsQ,
      scoredQ,
      avgQ,
    ...
    const totalApps = applications.rows[0].c;
    ...
    res.json({
      stats: {
        ...
        scored: scoredCount,
        pending: Math.max(0, totalApps - scoredCount),
        avgScore: avg.rows[0].a,
      },
    });
  } catch (err) {
...
// --- Конкурси, до яких призначено суддю --------------------------------------
app.get("/api/jury/competitions", juryOnly, async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT c.id, c.title, c.starts_at, c.ends_at,
              (SELECT COUNT(*)::int FROM applications a
                 WHERE a.competition_id = c.id) AS applications,
              (SELECT COUNT(DISTINCT res.application_id)::int
                 FROM results res
                 JOIN applications a ON a.id = res.application_id
                WHERE a.competition_id = c.id AND res.judge_id = $1) AS scored
         FROM competitions c
    ...
    console.log("[v0] Помилка конкурсів журі:", err.message);
    res.status(500).json({ error: "Внутрішня помилка серверу" });
  }
});
...
app.get("/api/jury/competitions/:id/applications", juryOnly, async (req, res) => {
  try {
    ...
    if (!cid) return res.status(400).json({ error: "Невірний конкурс" });
    if (!(await juryAssigned(req, cid))) {
      return res.status(403).json({ error: "Вас не призначено до цього конкурсу" });
    }

    const uid = req.user.id;
    const r = await pool.query(
      `SELECT a.id, a.title, a.data_json, a.status, a.created_at,
              sp.full_name AS student_name,
              s.name       AS section_name,
              (SELECT res.score   FROM results res
                 WHERE res.application_id = a.id AND res.judge_id = $2
                 ORDER BY res.created_at DESC LIMIT 1) AS my_score,
              (SELECT res.comment FROM results res
                 WHERE res.application_id = a.id AND res.judge_id = $2
                 ORDER BY res.created_at DESC LIMIT 1) AS my_comment
         FROM applications a
         LEFT JOIN user_profiles sp ON sp.user_id = a.student_id
         LEFT JOIN competition_sections s ON s.id = a.section_id
    ...
    const applications = r.rows;

    // Прикріплені файли заявок
    if (applications.length) {
      const ids = applications.map((a) => a.id);
      const files = await pool.query(
        `SELECT id, application_id, file_url, file_type
      ...
      for (const a of applications) a.files = byApp.get(a.id) || [];
    }

    res.json({ applications });
  } catch (err) {
    console.log("[v0] Помилка заявок журі:", err.message);
    res.status(500).json({ error: "Внутрішня помилка серверу" });
  }
});

// --- Виставлення / оновлення балу заявці --------------------------------------
app.post("/api/jury/applications/:id/score", juryOnly, async (req, res) => {
  try {
    const aid = parseInt(req.params.id, 10);
    if (!aid) return res.status(400).json({ error: "Невірна заявка" });

    const score = Number(req.body?.score);
    const comment = (req.body?.comment || "").trim() || null;
    if (!Number.isFinite(score) || score < 0 || score > 100) {
      return res.status(400).json({ error: "Бал має бути в межах 0–100" });
    }

    // Знаходимо заявку та її конкурс
    const a = await pool.query(
      "SELECT id, competition_id FROM applications WHERE id = $1",
      [aid]
    );
    if (a.rowCount === 0) return res.status(404).json({ error: "Заявку не знайдено" });
    if (!(await juryAssigned(req, a.rows[0].competition_id))) {
      return res.status(403).json({ error: "Вас не призначено до цього конкурсу" });
    }

    const uid = req.user.id;

    // Upsert: якщо суддя вже оцінював заявку — оновлюємо, інакше створюємо запис
    const existing = await pool.query(
      "SELECT id FROM results WHERE application_id = $1 AND judge_id = $2 ORDER BY created_at DESC LIMIT 1",
      [aid, uid]
    );
    let row;
    if (existing.rowCount > 0) {
      const upd = await pool.query(
        "UPDATE results SET score = $2, comment = $3, created_at = now() WHERE id = $1 RETURNING *",
        [existing.rows[0].id, score, comment]
      );
      row = upd.rows[0];
    } else {
      const ins = await pool.query(
        "INSERT INTO results (application_id, judge_id, score, comment) VALUES ($1,$2,$3,$4) RETURNING *",
        [aid, uid, score, comment]
      );
      row = ins.rows[0];
    }

    await logAction("Оцінено заявку", uid, `application #${aid} → ${score}`);
    res.status(201).json({
      message: existing.rowCount > 0 ? "Бал оновлено" : "Бал збережено",
      result: row,
    });
  } catch (err) {
    console.log("[v0] Помилка оцінювання:", err.message);
    res.status(500).json({ error: "Внутрішня помилка серверу" });
  }
});
